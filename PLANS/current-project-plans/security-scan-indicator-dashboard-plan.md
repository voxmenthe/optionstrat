...
Tasks:
1. Define Pydantic request/response models for indicator metadata and compute payloads.
2. Implement the adapter dataclasses/types.
3. Add metadata and compute adapters for `roc`.
4. Add `GET /security-scan/indicators`.
5. Add `POST /security-scan/indicator-dashboard/compute`.
6. Add backend tests using a fake `MarketDataService` or direct dependency override.
...
Tasks:
1. Add `securityScanApi.ts`.
2. Add the new Next.js page.
3. Render ticker/date/indicator controls.
4. Render schema-driven parameter fields for `roc`.
5. Add recompute flow with loading, error, and empty states.
6. Render a two-row Plotly chart with shared x-axis.
7. Add `localStorage` MRU strip.
...
## Open Questions
- Should the route path be `/security-scan/indicators`, `/screener/indicators`, or another naming convention?
...
- Should parameter edits auto-recompute with debounce, or require an explicit recompute button? The safer phase-1 default is explicit recompute.
...
1. Backend metadata + compute for `roc`.
2. Frontend page that can edit `roc_lookback`.
3. Plot close + ROC with signal markers.
4. MRU in `localStorage`.
5. Tests for the backend compute contract and MRU helper logic if frontend tests are available.
