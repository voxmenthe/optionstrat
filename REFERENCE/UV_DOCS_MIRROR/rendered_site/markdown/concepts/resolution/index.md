...
Most projects and packages have dependencies. Dependencies are other packages that are necessary in order for the current package to work. A package defines its dependencies as _requirements _, roughly a combination of a package name and acceptable versions. The dependencies defined by the current project are called _direct dependencies _. The dependencies added by each dependency of the current project are called _indirect _or _transitive dependencies _. 
...
Markers are important for resolution because their values change the required dependencies. Typically, Python package resolvers use the markers of the _current _platform to determine which dependencies to use since the package is often being _installed _on the current platform. However, for _locking _dependencies this is problematic — the lockfile would only work for developers using the same platform the lockfile was created on. To solve this problem, platform-independent, or "universal" resolvers exist. 
...
[#__codelineno-0-2](#__codelineno-0-2)environments = [
[#__codelineno-0-3](#__codelineno-0-3)    "sys_platform == 'darwin'",
[#__codelineno-0-4](#__codelineno-0-4)    "sys_platform == 'linux'",
...
[#__codelineno-1-2](#__codelineno-1-2)environments = [
[#__codelineno-1-3](#__codelineno-1-3)    "implementation_name == 'cpython'"
[#__codelineno-1-4](#__codelineno-1-4)]
...
Some packages (like PyTorch) publish built distributions, but omit a source distribution. Such packages are _only _installable on platforms for which a built distribution is available. For example, if a package publishes built distributions for Linux, but not macOS or Windows, then that package will _only _be installable on Linux. 
...
For example, `environments = ["sys_platform == 'darwin'"] `would limit uv to solving for macOS (and ignoring Linux and Windows). On the other hand, `required-environments = ["sys_platform == 'darwin'"] `would _require _that any package without a source distribution include a wheel for macOS in order to be installable (and would fail if no such wheel is available). 
...
[#__codelineno-2-2](#__codelineno-2-2)required-environments = [
[#__codelineno-2-3](#__codelineno-2-3)    "sys_platform == 'darwin' and platform_machine == 'x86_64'"
...
```
[#__codelineno-3-1](#__codelineno-3-1)flask>=2.0.0
...
```
[#__codelineno-4-1](#__codelineno-4-1)# This file was autogenerated by uv via the following command:
[#__codelineno-4-2](#__codelineno-4-2)#    uv pip compile requirements.in
[#__codelineno-4-3](#__codelineno-4-3)blinker==1.7.0
[#__codelineno-4-4](#__codelineno-4-4)    # via flask
[#__codelineno-4-5](#__codelineno-4-5)click==8.1.7
[#__codelineno-4-6](#__codelineno-4-6)    # via flask
[#__codelineno-4-7](#__codelineno-4-7)flask==3.0.0
[#__codelineno-4-8](#__codelineno-4-8)itsdangerous==2.1.2
[#__codelineno-4-9](#__codelineno-4-9)    # via flask
[#__codelineno-4-10](#__codelineno-4-10)jinja2==3.1.2
[#__codelineno-4-11](#__codelineno-4-11)    # via flask
[#__codelineno-4-12](#__codelineno-4-12)markupsafe==2.1.3
[#__codelineno-4-13](#__codelineno-4-13)    # via
[#__codelineno-4-14](#__codelineno-4-14)    #   jinja2
[#__codelineno-4-15](#__codelineno-4-15)    #   werkzeug
[#__codelineno-4-16](#__codelineno-4-16)werkzeug==3.0.1
[#__codelineno-4-17](#__codelineno-4-17)    # via flask
...
```
[#__codelineno-5-1](#__codelineno-5-1)# This file was autogenerated by uv via the following command:
[#__codelineno-5-2](#__codelineno-5-2)#    uv pip compile requirements.in --resolution lowest
[#__codelineno-5-3](#__codelineno-5-3)click==7.1.2
[#__codelineno-5-4](#__codelineno-5-4)    # via flask
[#__codelineno-5-5](#__codelineno-5-5)flask==2.0.0
[#__codelineno-5-6](#__codelineno-5-6)itsdangerous==2.0.0
[#__codelineno-5-7](#__codelineno-5-7)    # via flask
[#__codelineno-5-8](#__codelineno-5-8)jinja2==3.0.0
[#__codelineno-5-9](#__codelineno-5-9)    # via flask
[#__codelineno-5-10](#__codelineno-5-10)markupsafe==2.0.0
[#__codelineno-5-11](#__codelineno-5-11)    # via jinja2
[#__codelineno-5-12](#__codelineno-5-12)werkzeug==2.0.0
[#__codelineno-5-13](#__codelineno-5-13)    # via flask
...
2. If _all _published versions of a package are pre-releases. 
...
Pre-releases are [notoriously difficult](https://pubgrub-rs-guide.netlify.app/limitations/prerelease_versions)to model, and are a frequent source of bugs in other packaging tools. uv's pre-release handling is _intentionally _limited and requires user opt-in for pre-releases to ensure correctness. 
...
```
[#__codelineno-6-1](#__codelineno-6-1)numpy==1.24.4 ; python_version == "3.8"
[#__codelineno-6-2](#__codelineno-6-2)numpy==2.0.2 ; python_version == "3.9"
[#__codelineno-6-3](#__codelineno-6-3)numpy==2.2.0 ; python_version >= "3.10"
...
Dependency overrides allow bypassing unsuccessful or undesirable resolutions by overriding a package's declared dependencies. Overrides are a useful last resort for cases in which you _know _that a dependency is compatible with a certain version of a package, despite the metadata indicating otherwise. 

For example, if a transitive dependency declares the requirement `pydantic>=1.0, < 2.0 `, but _does _work with `pydantic>=2.0 `, the user can override the declared dependency by including `pydantic>=1.0, < 3 `in the overrides, thereby allowing the resolver to choose a newer version of `pydantic `. 
...
```
[#__codelineno-7-1](#__codelineno-7-1)[[tool.uv.dependency-metadata]]
[#__codelineno-7-2](#__codelineno-7-2)name = "chumpy"
[#__codelineno-7-3](#__codelineno-7-3)version = "0.70"
[#__codelineno-7-4](#__codelineno-7-4)requires-dist = ["numpy>=1.8.1", "scipy>=0.13.0", "six>=1.11.0"]

```

These declarations are intended for cases in which a package does _not _declare static metadata upfront, though they are also useful for packages that require [disabling build isolation](../projects/config/#build-isolation)In such cases, it may be easier to declare the package metadata upfront, rather than creating a custom build environment prior to resolving the package. 
...
[#__codelineno-8-4](#__codelineno-8-4)requires-python = ">=3.12"
[#__codelineno-8-5](#__codelineno-8-5)dependencies = ["flash-attn"]
[#__codelineno-8-6](#__codelineno-8-6)
[#__codelineno-8-7](#__codelineno-8-7)[tool.uv.sources]
[#__codelineno-8-8](#__codelineno-8-8)flash-attn = { git = "https://github.com/Dao-AILab/flash-attention", tag = "v2.6.3" }
[#__codelineno-8-9](#__codelineno-8-9)
[#__codelineno-8-10](#__codelineno-8-10)[[tool.uv.dependency-metadata]]
[#__codelineno-8-11](#__codelineno-8-11)name = "flash-attn"
[#__codelineno-8-12](#__codelineno-8-12)version = "2.6.3"
[#__codelineno-8-13](#__codelineno-8-13)requires-dist = ["torch", "einops"]

```

Like dependency overrides, `tool.uv.dependency-metadata `can also be used for cases in which a package's metadata is incorrect or incomplete, or when a package is not available in the package index. While dependency overrides allow overriding the allowed versions of a package globally, metadata overrides allow overriding the declared metadata of a _specific package _. 
...
```
[#__codelineno-9-1](#__codelineno-9-1)[project.optional-dependencies]
[#__codelineno-9-2](#__codelineno-9-2)extra1 = ["numpy==2.1.2"]
[#__codelineno-9-3](#__codelineno-9-3)extra2 = ["numpy==2.0.0"]
...
```
[#__codelineno-10-1](#__codelineno-10-1)$ uv lock
[#__codelineno-10-2](#__codelineno-10-2)  x No solution found when resolving dependencies:
[#__codelineno-10-3](#__codelineno-10-3)  `-> Because myproject[extra2] depends on numpy==2.0.0 and myproject[extra1] depends on numpy==2.1.2, we can conclude that myproject[extra1] and
[#__codelineno-10-4](#__codelineno-10-4)      myproject[extra2] are incompatible.
[#__codelineno-10-5](#__codelineno-10-5)      And because your project requires myproject[extra1] and myproject[extra2], we can conclude that your projects's requirements are unsatisfiable.
...
[#__codelineno-11-2](#__codelineno-11-2)conflicts = [
[#__codelineno-11-3](#__codelineno-11-3)    [
[#__codelineno-11-4](#__codelineno-11-4)      { extra = "extra1" },
[#__codelineno-11-5](#__codelineno-11-5)      { extra = "extra2" },
...
```
[#__codelineno-12-1](#__codelineno-12-1)$ uv sync --extra extra1 --extra extra2
[#__codelineno-12-2](#__codelineno-12-2)Resolved 3 packages in 14ms
[#__codelineno-12-3](#__codelineno-12-3)error: extra `extra1`, extra `extra2` are incompatible with the declared conflicts: {`myproject[extra1]`, `myproject[extra2]`}
...
```
[#__codelineno-13-1](#__codelineno-13-1)[dependency-groups]
[#__codelineno-13-2](#__codelineno-13-2)group1 = ["numpy==2.1.2"]
[#__codelineno-13-3](#__codelineno-13-3)group2 = ["numpy==2.0.0"]
[#__codelineno-13-4](#__codelineno-13-4)
[#__codelineno-13-5](#__codelineno-13-5)[tool.uv]
[#__codelineno-13-6](#__codelineno-13-6)conflicts = [
[#__codelineno-13-7](#__codelineno-13-7)    [
[#__codelineno-13-8](#__codelineno-13-8)      { group = "group1" },
[#__codelineno-13-9](#__codelineno-13-9)      { group = "group2" },
[#__codelineno-13-10](#__codelineno-13-10)    ],
[#__codelineno-13-11](#__codelineno-13-11)]
...
[#__codelineno-14-2](#__codelineno-14-2)name = "member1"
[#__codelineno-14-3](#__codelineno-14-3)
[#__codelineno-14-4](#__codelineno-14-4)[project.optional-dependencies]
[#__codelineno-14-5](#__codelineno-14-5)extra1 = ["numpy==2.1.2"]
...
[#__codelineno-15-2](#__codelineno-15-2)name = "member2"
[#__codelineno-15-3](#__codelineno-15-3)
[#__codelineno-15-4](#__codelineno-15-4)[project.optional-dependencies]
[#__codelineno-15-5](#__codelineno-15-5)extra2 = ["numpy==2.0.0"]
...
```
[#__codelineno-16-1](#__codelineno-16-1)[tool.uv]
[#__codelineno-16-2](#__codelineno-16-2)conflicts = [
[#__codelineno-16-3](#__codelineno-16-3)    [
[#__codelineno-16-4](#__codelineno-16-4)      { package = "member1", extra = "extra1" },
[#__codelineno-16-5](#__codelineno-16-5)      { package = "member2", extra = "extra2" },
[#__codelineno-16-6](#__codelineno-16-6)    ],
...
[#__codelineno-17-2](#__codelineno-17-2)name = "member1"
[#__codelineno-17-3](#__codelineno-17-3)dependencies = ["numpy==2.1.2"]
...
[#__codelineno-18-2](#__codelineno-18-2)name = "member2"
[#__codelineno-18-3](#__codelineno-18-3)
[#__codelineno-18-4](#__codelineno-18-4)[project.optional-dependencies]
[#__codelineno-18-5](#__codelineno-18-5)extra2 = ["numpy==2.0.0"]
...
```
[#__codelineno-19-1](#__codelineno-19-1)[tool.uv]
[#__codelineno-19-2](#__codelineno-19-2)conflicts = [
[#__codelineno-19-3](#__codelineno-19-3)    [
[#__codelineno-19-4](#__codelineno-19-4)      { package = "member1" },
[#__codelineno-19-5](#__codelineno-19-5)      { package = "member2", extra = "extra2" },
...
```
[#__codelineno-20-1](#__codelineno-20-1)[project]
[#__codelineno-20-2](#__codelineno-20-2)name = "member1"
[#__codelineno-20-3](#__codelineno-20-3)dependencies = ["numpy==2.1.2"]
...
```
[#__codelineno-21-1](#__codelineno-21-1)[project]
[#__codelineno-21-2](#__codelineno-21-2)name = "member2"
[#__codelineno-21-3](#__codelineno-21-3)dependencies = ["numpy==2.0.0"]
...
[#__codelineno-22-4](#__codelineno-22-4)      { package = "member1" },
[#__codelineno-22-5](#__codelineno-22-5)      { package = "member2" },
...
```
[#__codelineno-23-1](#__codelineno-23-1)[project]
[#__codelineno-23-2](#__codelineno-23-2)name = "root"
[#__codelineno-23-3](#__codelineno-23-3)dependencies = ["member1", "member2"]
...
!!! important "Important"
    ...
    The package index must support the `upload-time `field as specified in [`PEP 700 `](https://peps.python.org/pep-0700/). If the field is not present for a given distribution, the distribution will be treated as unavailable unless the package is opted out via `--exclude-newer-package =false `. PyPI provides `upload-time `for all packages. 
...
[#__codelineno-24-2](#__codelineno-24-2)exclude-newer = "2006-12-02T02:07:43Z"
...
[#__codelineno-25-2](#__codelineno-25-2)exclude-newer-package = { setuptools = "2006-12-02T02:07:43Z" }

```

The same flag also accepts `=false `to opt a package out of the `--exclude-newer `restriction, e.g., to allow resolving packages from an index that does not publish upload times. 
...
```
[#__codelineno-26-1](#__codelineno-26-1)[tool.uv]
[#__codelineno-26-2](#__codelineno-26-2)exclude-newer = "1 week"
...
[#__codelineno-27-2](#__codelineno-27-2)exclude-newer = "1 week"
[#__codelineno-27-3](#__codelineno-27-3)exclude-newer-package = { setuptools = "30 days" }
...
uv versions that support schema v2 _may _be able to read lockfiles with schema v1 if the schema update was backwards-compatible. However, this is not guaranteed, and uv may exit with an error if it encounters a lockfile with an outdated schema version. 
