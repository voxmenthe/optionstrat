...
Since it is common for a system to have an existing Python installation, uv supports [discovering](#discovery-of-python-versions)Python versions. However, uv also supports [installing Python versions](#installing-a-python-version)itself. To distinguish between these two types of Python installations, uv refers to Python versions it installs as _managed _Python installations and all other Python installations as _system _Python installations. 

!!! note "Note"

    uv does not distinguish between Python versions installed by the operating system vs those installed and managed by other tools. For example, if a Python installation is managed with `pyenv `, it would still be considered a _system _Python version in uv. 
...
```
[#__codelineno-0-1](#__codelineno-0-1)$ uv venv --python 3.11.6
...
```
[#__codelineno-1-1](#__codelineno-1-1)$ uv python install 3.12.3
...
```
[#__codelineno-2-1](#__codelineno-2-1)$ uv python install 3.12
...
```
[#__codelineno-3-1](#__codelineno-3-1)$ uv python install '>=3.8,<3.10'
...
```
[#__codelineno-4-1](#__codelineno-4-1)$ uv python install 3.9 3.10 3.11
...
```
[#__codelineno-5-1](#__codelineno-5-1)$ uv python install pypy
...
```
[#__codelineno-6-1](#__codelineno-6-1)$ uv python install 3.12 --default
...
```
[#__codelineno-7-1](#__codelineno-7-1)$ uv python install 3.12.7  # Adds `python3.12` to `~/.local/bin`
[#__codelineno-7-2](#__codelineno-7-2)$ uv python install 3.12.6  # Does not update `python3.12`
[#__codelineno-7-3](#__codelineno-7-3)$ uv python install 3.12.8  # Updates `python3.12` to point to 3.12.8
...
!!! important "Important"
    ...
    Support for upgrading Python versions is in _preview _. This means the behavior is experimental and subject to change. 
...
```
[#__codelineno-8-1](#__codelineno-8-1)$ uv python upgrade 3.12
...
```
[#__codelineno-9-1](#__codelineno-9-1)$ uv python upgrade
...
!!! note "Note"
    ...
    If the virtual environment was created _before _opting in to the preview mode, it will not be included in the automatic upgrades. 
...
```
[#__codelineno-10-1](#__codelineno-10-1)~/.local/share/uv/python/cpython-3.12-macos-aarch64-none
...
```
[#__codelineno-11-1](#__codelineno-11-1)$ readlink ~/.local/share/uv/python/cpython-3.12-macos-aarch64-none
[#__codelineno-11-2](#__codelineno-11-2)~/.local/share/uv/python/cpython-3.12.11-macos-aarch64-none
...
```
[#__codelineno-12-1](#__codelineno-12-1)$ uv python list
...
```
[#__codelineno-13-1](#__codelineno-13-1)$ uv python list 3.13
...
```
[#__codelineno-14-1](#__codelineno-14-1)$ uv python list pypy
...
```
[#__codelineno-15-1](#__codelineno-15-1)$ uv python list --all-versions
...
```
[#__codelineno-16-1](#__codelineno-16-1)$ uv python list --all-platforms
...
```
[#__codelineno-17-1](#__codelineno-17-1)$ uv python list --only-installed
...
```
[#__codelineno-18-1](#__codelineno-18-1)$ uv python find
...
```
[#__codelineno-19-1](#__codelineno-19-1)$ uv python find '>=3.11'
...
```
[#__codelineno-20-1](#__codelineno-20-1)$ uv python find --system
...
```
[#__codelineno-21-1](#__codelineno-21-1)$ uv python list --managed-python
...
```
[#__codelineno-22-1](#__codelineno-22-1)$ uv python list --no-managed-python
...
```
[#__codelineno-23-1](#__codelineno-23-1)$ uv python install 3.13.1
[#__codelineno-23-2](#__codelineno-23-2)$ py -V:Astral/CPython3.13.1
