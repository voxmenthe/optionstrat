...
This is the updated "v2" logic that replaces the hard validity/sign gates
with a continuous confidence weighting to reduce long all-zero plateaus.
...
def qrs_consist_excess(
    ...
    day_score_sum = _rolling_sum(day_score, lookback)
    consistency = [(day_score_sum[i] / _safe_max(active_day_count[i], 1.0)) for i in range(n)]
    ...
    excess_norm = [raw_excess[i] / _safe_max(excess_std[i], 0.1) for i in range(n)]

    combined = [(consistency[i] * cons_weight) + (excess_norm[i] * excess_weight) for i in range(n)]

    # Confidence weighting (soft gates) to avoid long all-zero plateaus.
    min_active_days = lookback * 0.4
    min_active_days_floor = max(min_active_days, 1.0)
    sample_conf = [min(active_day_count[i] / min_active_days_floor, 1.0) for i in range(n)]
    ...
    confidence = [sample_conf[i] * balance_conf[i] * align_conf[i] for i in range(n)]
    ...
    return {
        # The .txt currently names the main output line QRSConsistExcessV2,
        # but we keep the historical key as QRSConsistExcess as well.
